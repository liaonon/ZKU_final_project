...
template checkWall(n) {
    ...
    // check if the wall exist or not
    component isEqual[2*n];
    var outNumber = 0;
    for(var i=0; i<n; i++){
        isEqual[i] = IsEqual();
        isEqual[i].in[0] <== xwallsA[i];
        isEqual[i].in[1] <== xpathWall;

        isEqual[i+n] = IsEqual();
        isEqual[i+n].in[0] <== xwallsB[i];
        isEqual[i+n].in[1] <== xpathWall;

        outNumber = outNumber + isEqual[i].out + isEqual[i+n].out;
    }
